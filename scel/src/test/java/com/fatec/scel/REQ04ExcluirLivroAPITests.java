...
class REQ04ExcluirLivroAPITests {
	...
	void ct01_quando_exclui_pelo_id_consulta_por_isbn_deve_retorna_null() {
		...
		ResponseEntity<String> resposta = testRestTemplate.exchange(urlBase + umLivro.getId(), HttpMethod.DELETE, null, String.class);
	...
	@Test
	void ct02_quando_exclui_pelo_id_invalido_retorna_bad_request() {
		// Dado – que o id está cadastrado
		repository.deleteAll();
		// Quando – o usuário solicita exclusao
		ResponseEntity<String> resposta = testRestTemplate.exchange(urlBase + "99" , HttpMethod.DELETE, null, String.class);
